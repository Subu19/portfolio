...
})({"8ctZJ":[function(require,module,exports) {
"use strict";
...
module.bundle.HMR_BUNDLE_ID = "727f764846a420a3";
...
/** @type {HTMLCanvasElement} */ var _globalVeriable = require("./globalVeriable");
...
class Ball {
    constructor(){
        ...
        this.expired = false;
        this.duration = Math.floor(Math.random() * 1501) + 500;
    ...
    update() {
        ...
        if (this.stage == 1) {
            this.currentRadius < this.radius ? this.currentRadius += 0.2 : this.stage = 2;
        ...
        if (this.stage == 3) {
            ...
            this.line.x = this.line.x + (this.destination.x - this.line.x) * deltaTime;
            this.line.y = this.line.y + (this.destination.y - this.line.y) * deltaTime;
            if (deltaTime >= 1) {
        ...
        if (this.stage == 5) {
            this.currentRadius < 20 ? this.currentRadius += 0.5 : this.expired = true;
            ctx.arc(this.x, this.y, this.currentRadius, 0, 2 * Math.PI);
            ctx.stroke();
        }
    //update veriables
...
function updateBalls() {
    for(let i = 0; i < balls.length; i++){
        const ball = balls[i];
        if (ball.expired) balls.splice(i, 1);
        else ball.update();
    }
}
function animatePullEffect() {
    ...
    ctx.fillStyle = (0, _globalVeriable.colors).currentBack;
    ctx.fill();
...
function makeStickyEffect(follow) {
    ...
    if (follow == true) {
        ...
        realoffset < offset ? offset -= 1 : realoffset > offset ? offset += 1 : offset = realoffset;
    } else if (offset != 0) {
    ...
    for(let i1 = mouse.y; i1 < c.height; i1++){
        angle = angle + x;
        ctx.lineTo(100 + Math.sin(toRadient(angle * 0.5)) * offset, i1);
    }
}

},{"./globalVeriable":"acJy3"}]},["8ctZJ","jvBLU"], "jvBLU", "parcelRequire6913")

//# sourceMappingURL=index.46a420a3.js.map
