(function($) {
    $.fn.connections = function(options) {
        if (options === "update") return processConnections(update, this);
        else if (options === "remove") return processConnections(destroy, this);
        else {
            options = $.extend(true, {
                borderClasses: {},
                class: "connection",
    ...
    var createConnection = function(container, nodes, tag, borderClasses, options) {
        ...
        if (border_w <= 0 && border_h <= 0) border_w = border_h = 1;
        var data = {
        ...
        if ("none" === connection.css("border-top-style")) data.css.borderStyle = "solid";
        $.data(connection.get(0), "connection", data);
        $.data(connection.get(0), "connections", [
            connection.get(0)
        ]);
        for(var i = 0; i < 2; i++){
            var connections = connection.add($.data(nodes[i], "connections")).get();
            $.data(nodes[i], "connections", connections);
            if (connections.length == 1) $(nodes[i]).on("connections.connections", false);
        }
    ...
    var getState = function(data) {
        ...
        if (cached === undefined) return data.unmodified = false;
        for(var i = 0; i < 8; i++){
            if (cached[i] !== data.cache[i]) return data.unmodified = false;
        }
    };
    var update = function(connection) {
        ...
        if (data.hidden) style += "display: none;";
        else {
    ...
    var processConnections = function(method, elements) {
        return elements.each(function() {
            ...
            if (connections instanceof Array) for(var i = 0, len = connections.length; i < len; i++)method(connections[i]);
        });
    };
})(jQuery);

//# sourceMappingURL=index.7dfc5c48.js.map
